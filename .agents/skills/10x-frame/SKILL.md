...
- **Use sub-agents when the surface is large or unfamiliar.** Delegate one or two exploration tasks with prompts like: "Trace the path from <stated cause> to <observed effect>. List every distinct stage or axis the chain passes through, with file:line or document:section references." The map is what they return — not what you guessed before reading.
...
Create one task per plausible dimension. Then delegate parallel investigation tasks — typically 2–4, capped at 5 — concurrently.
...
- Use an exploration-focused sub-agent for "find the code or document that handles X, show me the structure".
- Use a general-purpose sub-agent for "trace this chain and tell me whether assumption Y holds".
...
- **Independent search.** Delegate a fresh exploration task with a prompt that does NOT name the leading hypothesis. Describe only the observation and ask: "What in this system or design space is most likely responsible? Look without preconception." If the sub-agent independently lands on the same hypothesis, confidence rises. If it surfaces something different, that's a signal worth reading carefully.
...
- **Header:** "Next step"
- **Options:**
  - **Hand off to /10x-plan:** Pass this brief to /10x-plan and start implementation planning.
