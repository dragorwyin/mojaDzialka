...
host's actual structured question capability when available, respecting its schema,
round size and custom input. Use short headers (at most 12 characters) and concrete
options with descriptions. In a host without that capability, ask the user a concise
text question for required scope clarification; do not simulate a tool call. An
unambiguous research request needs no interview or mode switch. User preferences are
decisions; verify corrected factual assertions against the indicated source.
...
Use your AI coding assistant's native sub-agent or task capability for locating code
and analysis, selecting the appropriate available agent type. Use task tracking when
it helps coordinate several areas and the host exposes it; otherwise the working
list suffices.
