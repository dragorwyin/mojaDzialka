...
5. No argument → enumerate `context/changes/*/change.md`; pick the most recently `updated` change with `status` in `{implementing, implemented}` and ask the user to confirm.
...
Create a task: "Implementation Review" with active status "Loading context".
...
Don't pre-read every changed file into the main context — let parallel review agents read what they need. Main context should carry the plan and the diff summary, not the full source of 20 files.

## Step 2: Parallel review via review agents

Update the task status to "Gathering evidence".

Launch **two** parallel review agents simultaneously. Each gets targeted context — don't dump the full plan into both.

**Agent 1 — Plan Drift Detection** (`general-purpose`)
...
**Agent 2 — Safety, Quality & Pattern Compliance** (`general-purpose`)
...
Update the task status to "Verifying success criteria".

For each reviewed phase:

**Automated**: run each command from the "Automated Verification" checkboxes in a shell. Record command, pass/fail, actual output (truncate if huge).
...
Update the task status to "Compiling findings".
...
With the report already saved and `change.md` already stamped, ask the user:

**Review saved to `<report-path>`. How would you like to proceed?**

**Implementation Review — [N] findings**

- **Triage findings now**: Walk through each finding and decide. Decisions are written back to the saved report.
- **Triage later**: Resume with `/10x-impl-review <report-path>`.
- **Done**: Report saved — I'll handle the findings myself.
...
Update the task status to "Triage".
...
- **Record as lesson**: Save as a recurring project rule via `/10x-lesson`.
...
- **Record as lesson**: pre-fill four lessons-entry fields directly from the finding — `Context` from the finding's Location, `Problem` from the finding's Detail, `Rule` and `Applies to` left as empty placeholders for the user to fill. Show the proposed entry as a complete markdown block and ask the user to edit or confirm: "Approve this entry?" with options "Approve this entry?", "Edit before saving", or "Cancel". On confirm, append the entry as a new H2 section to `context/foundation/lessons.md` — if the file does not exist, create it first with this canonical 5-line header (no separate template file; the header is embedded inline here):
